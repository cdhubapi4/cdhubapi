...
function startGameListeners(e) {
  startGame(e);
  removeStartGameListeners();
}

function removeStartGameListeners() {
  game.removeEventListener("keydown", startGameListeners);
  game.removeEventListener("click", startGameListeners);
  game.removeEventListener("touchstart", startGameListeners);
}

function addStartGameListeners() {
  game.addEventListener("keydown", startGameListeners, { once: true });
  game.addEventListener("click", startGameListeners, { once: true });
  game.addEventListener("touchstart", startGameListeners, { once: true });
}
...
function setDinoLose() {
  dino.src = "/lib/dino-game/assets/dino-lose.png";
}

function handleRun(delta, speedScale) {
  if (isJumping) {
    dino.src = `/lib/dino-game/assets/dino-stationary.png`;
    return;
  }

  if (currentFrameTime >= FRAME_TIME) {
    dinoFrame = (dinoFrame + 1) % DINO_FRAME_COUNT;
    dino.src = `/lib/dino-game/assets/dino-run-${dinoFrame}.png`; /* switch between images to simulate movement */
...
function createCactus() {
  ...
  cactus.src = "/lib/dino-game/assets/cactus.png";
