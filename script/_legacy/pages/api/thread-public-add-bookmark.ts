...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    ...
    //#region query excute
    // insert bookmark
    const query1 = `INSERT IGNORE INTO thread_public_bookmark (thread_id, user_id) VALUES (?, ?);`;

    // update bookmark_count + 1
    const query2 = `UPDATE thread_public SET bookmark_count = bookmark_count + 1 WHERE thread_id = ?;`;
