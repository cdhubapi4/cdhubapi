...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    ...
    //#region query excute
    // get my bookmark
    const query1 = `SELECT COUNT(user_id) > 0 is_checked FROM thread_public_bookmark WHERE thread_id = ? AND user_id = ?;`;
    const result1 = await db.query<{ is_checked: 0 | 1 }[]>(query1, [thread_id, user_id, thread_id]);
