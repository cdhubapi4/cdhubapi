...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    ...
    const deleteQuery = `DELETE FROM user_block WHERE (user_id = ? AND target_user_id = ?) OR (user_id = ? AND target_user_ip = ?)`;
    const result = await db.query(deleteQuery, [user_id, target_user_id, user_id, target_user_ip]);
