...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    ...
    if (typeof gender !== "string") return res.status(400).json({ result: null, error: "no params: gender" });
    //#endregion

    const genderValue = gender === "M" ? "male" : "female";
    //#region query excute
    const params = [genderValue, user_id];
    const query = "UPDATE user SET gender=? where user_id = ? and deleted_at is null;";
    const result = await db.query(query, params);

    res.setHeader(
      "Set-Cookie",
      `key=${encrypt({ ...userData, gender: genderValue } as object)}; Path=/; HttpOnly; Secure; SameSite=Strict`
    );
